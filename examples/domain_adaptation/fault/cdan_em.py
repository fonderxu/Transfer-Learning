...
from dalib.adaptation.cdan import ConditionalDomainAdversarialLoss, FaultClassifier
...
def main(args: argparse.Namespace):
    ...
    classifier = FaultClassifier(backbone, num_classes, bottleneck_dim=args.bottleneck_dim,
    ...
    for epoch in range(args.epochs):
        ...
        # train for one epoch
        train(train_source_iter, train_target_iter, classifier, s_label_distribution, t_output_distribution, domain_adv,
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator, model: FaultClassifier,
          s_label_distribution, t_output_distribution, domain_adv: ConditionalDomainAdversarialLoss, optimizer: SGD,
    ...
    lam = 0.1 * (2 / (1 + math.exp(-1. * 10. * epoch / args.epochs)) - 1)
    ...
    for i in range(args.iters_per_epoch):
        ...
        y_t_softmax = torch.softmax(y_t, dim=1)
        target_entropy_loss = -torch.mean((y_t_softmax * torch.log(y_t_softmax + 1e-6)).sum(dim=1))

        weight = utils.get_adv_weight(s_label_distribution, t_output_distribution, labels_s, y_t_softmax)
        transfer_loss = domain_adv(y_s, f_s, y_t, f_t, weight)
        domain_acc = domain_adv.domain_discriminator_accuracy
        loss = cls_loss + transfer_loss * args.trade_off + lam * target_entropy_loss
