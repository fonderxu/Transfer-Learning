...
def main(args: argparse.Namespace):
    ...
    len_feature_extractor = len(list(classifier.backbone.parameters()) + list(classifier.pool_layer.parameters()) + list(classifier.bottleneck.parameters()))
    ...
    for epoch in range(args.epochs):
        ...
        train(train_source_iter, train_target_iter, classifier, weighted_adv_loss, s_label_distribution,
              t_output_distribution, len_feature_extractor, optimizer, lr_scheduler, num_classes, epoch, args)
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator, model: FaultClassifier,
          weighted_adv_loss: nn.Module, s_label_distribution, t_output_distribution, len_feature_extractor: int, optimizer: SGD,
    ...
    lam = 0.1 * (2 / (1 + math.exp(-1. * 10. * (epoch - args.epochs / 2) / (args.epochs / 2))) - 1) if epoch >= (args.epochs / 2) else 0
    ...
    for i in range(args.iters_per_epoch):
        ...
        for index, param in enumerate(model.parameters()):
            if param.grad is not None:
                if index < len_feature_extractor:
