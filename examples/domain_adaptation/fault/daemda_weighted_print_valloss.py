...
def main(args: argparse.Namespace):
    ...
    if args.phase == 'analysis':
        ...
        feature_extractor = nn.Sequential(classifier.backbone).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        train(train_source_iter, train_target_iter, classifier, weighted_adv_loss, s_label_distribution,
              t_output_distribution, len_feature_extractor, optimizer, lr_scheduler, num_classes, epoch, args, test_loader)
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator, model: FaultClassifier,
          ...
          lr_scheduler: LambdaLR, num_classes: int, epoch: int, args: argparse.Namespace, test_loader):
    ...
    for i in range(args.iters_per_epoch):
        ...
        loss = cls_loss + lam * target_entropy_loss
        ...
        optimizer.zero_grad()
        # 1.
        (transfer_loss * args.trade_off).backward(retain_graph=True)
        adv_grad = []
        ...
        with torch.no_grad():
            for index, (images, target) in enumerate(test_loader):
                if index == 0:
                    images_all = images.to(device)
                    target_all = target.to(device)
                else:
                    images_all = torch.cat((images_all, images.to(device)), 0)
                    target_all = torch.cat((target_all, target.to(device)), 0)
            # compute output
            _, (output, _) = model(images_all)
            loss_ = F.cross_entropy(output, target_all)
            print(i + epoch * args.iters_per_epoch + 1, ',', loss_.item())
