...
sys.path.append('../../..')
from dalib.adaptation.dmdd_adam import FaultClassifier, DomainAdversarial
...
def main(args: argparse.Namespace):
    ...
    weighted_adv_loss = DomainAdversarial().to(device)
    ...
    for epoch in range(args.epochs):
        ...
        # train for one epoch
        train(train_source_iter, train_target_iter, classifier, weighted_adv_loss, len_backbone, optimizer,
              lr_scheduler, num_classes, epoch, args)
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator, model: FaultClassifier,
          weighted_adv_loss: nn.Module, len_backbone: int, optimizer: SGD, lr_scheduler: LambdaLR, num_classes: int,
    ...
    for i in range(args.iters_per_epoch):
        ...
        # # compute output
        feature_s, y_s = model(x_s)
        feature_t, y_t = model(x_t)

        cls_loss = F.cross_entropy(y_s, labels_s)

        mmd_loss_feature = utils.mmd_linear(feature_s, feature_t)
        mmd_loss_pred = utils.mmd_linear(F.log_softmax(y_s), F.log_softmax(y_t))

        transfer_loss = weighted_adv_loss(feature_s, feature_t)

        loss = cls_loss + transfer_loss + 0.25 * (mmd_loss_feature + mmd_loss_pred)
